...
export function solveSatan(data) {
  if (typeof data !== "string") return "";
  const hexArray = data.match(/[0-9a-f]+/gi);
  if (!hexArray || hexArray.length === 0) return "";
  for (let i = 0; i < hexArray.length; i += 17) {
    hexArray[i] = undefined;
  }
  const clearArray = hexArray.filter(Boolean).join("");
  let position = -1;
  ...
  return row.toString(16).padStart(2, "0") + column.toString(16);
}
